...
c    -----------------------------------------
      ...
      do j=js,je-1
      do i=is,ie
         a2 = gm*qLy(i,j,8)/qLy(i,j,1)
         aas2 = (gm - 1.d0)*(qLy(i,j,10) - 0.5d0*(qLy(i,j,2)**2
     &                     + qLy(i,j,3)**2 + qLy(i,j,4)**2))
     &        - (gm - 2.d0)*pi4i*(qLy(i,j,5)**2 + qLy(i,j,6)**2
     &                          + qLy(i,j,7)**2)/qLy(i,j,1)
         if(aas2 .lt. 0.d0) aas2 = 0.d0
         valfvL(i,j) = dabs(qLy(i,j,5))/dsqrt(pi4*qLy(i,j,1))
...
C Flux interpolated from the left-hand side
         fyL(i,j,1) = qLy(i,j,1)*qLy(i,j,2)
         fyL(i,j,2) = qLy(i,j,1)*qLy(i,j,2)**2 + qLy(i,j,8)
     &              + 0.5d0*pi4i*(qLy(i,j,6)**2 + qLy(i,j,7)**2
     &                          - qLy(i,j,5)**2)
         fyL(i,j,3) = qLy(i,j,1)*qLy(i,j,2)*qLy(i,j,3)
     &              - pi4i*qLy(i,j,5)*qLy(i,j,6)
         fyL(i,j,4) = qLy(i,j,1)*qLy(i,j,2)*qLy(i,j,4)
     &              - pi4i*qLy(i,j,5)*qLy(i,j,7)
         fyL(i,j,5) = qLy(i,j,2)*qLy(i,j,6) - qLy(i,j,3)*qLy(i,j,5)
         fyL(i,j,6) = qLy(i,j,2)*qLy(i,j,7) - qLy(i,j,4)*qLy(i,j,5)
         fyL(i,j,7) = qLy(i,j,1)*qLy(i,j,10)*qLy(i,j,2)
     &              - pi4i*qLy(i,j,5)*
     &               (qLy(i,j,5)*qLy(i,j,2) + qLy(i,j,6)*qLy(i,j,3)
     &              + qLy(i,j,7)*qLy(i,j,4))

         a2 = gm*qRy(i,j,8)/qRy(i,j,1)
         aas2 = (gm - 1.d0)*(qRy(i,j,10) - 0.5d0*(qRy(i,j,2)**2
     &                     + qRy(i,j,3)**2 + qRy(i,j,4)**2))
     &        - (gm - 2.d0)*pi4i*(qRy(i,j,5)**2 + qRy(i,j,6)**2
     &                          + qRy(i,j,7)**2)/qRy(i,j,1)
         if(aas2 .lt. 0.d0) aas2 = 0.d0
         valfvR(i,j) = dabs(qRy(i,j,5))/dsqrt(pi4*qRy(i,j,1))
...
C Flux interpolated from the right-hand side
         fyR(i,j,1) = qRy(i,j,1)*qRy(i,j,2)
         fyR(i,j,2) = qRy(i,j,1)*qRy(i,j,2)**2 + qRy(i,j,8)
     &              + 0.5d0*pi4i*(qRy(i,j,6)**2 + qRy(i,j,7)**2
     &                          - qRy(i,j,5)**2)
         fyR(i,j,3) = qRy(i,j,1)*qRy(i,j,2)*qRy(i,j,3)
     &              - pi4i*qRy(i,j,5)*qRy(i,j,6)
         fyR(i,j,4) = qRy(i,j,1)*qRy(i,j,2)*qRy(i,j,4)
     &              - pi4i*qRy(i,j,5)*qRy(i,j,7)
         fyR(i,j,5) = qRy(i,j,2)*qRy(i,j,6) - qRy(i,j,3)*qRy(i,j,5)
         fyR(i,j,6) = qRy(i,j,2)*qRy(i,j,7) - qRy(i,j,4)*qRy(i,j,5)
         fyR(i,j,7) = qRy(i,j,1)*qRy(i,j,10)*qRy(i,j,2)
     &              - pi4i*qRy(i,j,5)*
     &               (qRy(i,j,5)*qRy(i,j,2) + qRy(i,j,6)*qRy(i,j,3)
     &              + qRy(i,j,7)*qRy(i,j,4))
      ...
      do j=js,je-1
      do i=is,ie
         sqro1 = dsqrt(qLy(i,j,1))
         sqro2 = dsqrt(qRy(i,j,1))
c Arithmetical mean of density
         armroi = 1.d0/(sqro1 + sqro2)
C Roe average
         avero = dsqrt(qLy(i,j,1)*qRy(i,j,1))
         avevx = (sqro1*qLy(i,j,2)  + sqro2*qRy(i,j,2))*armroi
         avevy = (sqro1*qLy(i,j,3)  + sqro2*qRy(i,j,3))*armroi
         avevz = (sqro1*qLy(i,j,4)  + sqro2*qRy(i,j,4))*armroi
         avebx = (sqro1*qRy(i,j,5)  + sqro2*qLy(i,j,5))*armroi
         aveby = (sqro1*qRy(i,j,6)  + sqro2*qLy(i,j,6))*armroi
         avebz = (sqro1*qRy(i,j,7)  + sqro2*qLy(i,j,7))*armroi
         aveei = (sqro1*qLy(i,j,9)  + sqro2*qRy(i,j,9))*armroi
         aveen = (sqro1*qLy(i,j,10) + sqro2*qRy(i,j,10))*armroi
         avepr = (sqro1*qLy(i,j,8)  + sqro2*qRy(i,j,8))*armroi

         aveke = 0.5d0*(avevx**2 + avevy**2 + avevz**2)

C Characteristic velocity
         delb2 = (gm - 2.d0)/(gm - 1.d0)*0.5d0*pi4i*
     &           ((qRy(i,j,6) - qLy(i,j,6))**2
     &          + (qRy(i,j,7) - qLy(i,j,7))**2)*armroi**2
...
C Eigen value
         ...
         eps(1) = dmax1(0.d0,dabs(eigen(1) - (qLy(i,j,2) + vfastL(i,j)))
     &                 ,dabs(qRy(i,j,2) + vfastR(i,j+1) - eigen(1)))
         ...
         eps(2) = dmax1(0.d0,dabs(eigen(2) - (qLy(i,j,2) + valfvL(i,j)))
     &                 ,dabs(qRy(i,j,2) + valfvR(i,j) - eigen(2)))
         ...
         eps(3) = dmax1(0.d0,dabs(eigen(3) - (qLy(i,j,2) + vslowL(i,j)))
     &                 ,dabs(qRy(i,j,2) + vslowR(i,j+1) - eigen(3)))
         ...
         eps(4) = dmax1(0.d0,dabs(eigen(4) - qLy(i,j,2)),
     &                  dabs(qRy(i,j,2) - eigen(4)))
         ...
         eps(5) = dmax1(0.d0,dabs(eigen(5) - (qLy(i,j,2) - vslowL(i,j)))
     &                 ,dabs(qRy(i,j,2) - vslowR(i,j+1) - eigen(5)))
         ...
         eps(6) = dmax1(0.d0,dabs(eigen(6) - (qLy(i,j,2) - valfvL(i,j)))
     &                 ,dabs(qRy(i,j,2) - valfvR(i,j) - eigen(6)))
         ...
         eps(7) = dmax1(0.d0,dabs(eigen(7) - (qLy(i,j,2) - vfastL(i,j)))
     &                 ,dabs(qRy(i,j,2) - vfastR(i,j+1) - eigen(7)))
...
C Difference of physical variable
         delro = qRy(i,j,1) - qLy(i,j,1)
         delvx = qRy(i,j,2) - qLy(i,j,2)
         delvy = qRy(i,j,3) - qLy(i,j,3)
         delvz = qRy(i,j,4) - qLy(i,j,4)
         delby = qRy(i,j,6) - qLy(i,j,6)
         delbz = qRy(i,j,7) - qLy(i,j,7)
         delpr = qRy(i,j,8) - qLy(i,j,8)
...
C Numerical Flux
         fy(i,j,1) = 0.5d0*(fyL(i,j,1) + fyR(i,j,1))
         fy(i,j,2) = 0.5d0*(fyL(i,j,2) + fyR(i,j,2))
         fy(i,j,3) = 0.5d0*(fyL(i,j,3) + fyR(i,j,3))
         fy(i,j,4) = 0.5d0*(fyL(i,j,4) + fyR(i,j,4))
         fy(i,j,5) = 0.5d0*(fyL(i,j,5) + fyR(i,j,5))
         fy(i,j,6) = 0.5d0*(fyL(i,j,6) + fyR(i,j,6))
         fy(i,j,7) = 0.5d0*(fyL(i,j,7) + fyR(i,j,7))
     ...
     &                - 0.5d0*dabs(eigen(l))*anpw(l)*Rev(7,l)
         ...
         if(nstep .eq. 2) then
            delro = qRy(i,j+1,1) - qLy(i,j+1,1)
            delvx = qRy(i,j+1,2) - qLy(i,j+1,2)
            delvy = qRy(i,j+1,3) - qLy(i,j+1,3)
            delvz = qRy(i,j+1,4) - qLy(i,j+1,4)
            delby = qRy(i,j+1,6) - qLy(i,j+1,6)
            delbz = qRy(i,j+1,7) - qLy(i,j+1,7)
            delpr = qRy(i,j+1,8) - qLy(i,j+1,8)
